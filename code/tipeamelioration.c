...
// structures
struct Noeud 
{ //rajouter un indice a Node pour définir une fonction de poid
    int* indexDonnee;  //tableau contenant les indices des données possédés
...
struct AbrDecision 
{// rajouter valuation et NOM en int
    int profondeurMax;         //Permet de connaitre la profondeur maximum de parcous
    ...
    int* oobIndices;                   //tableau des indices out of bag pour cet arbre
...
{//indices contient les indices dans table de l'ensemble qu'on considére, card son cardinal, param l'indice du paramètre par rapport auquel on considére le split
    ...
    int plus = 0;
    int egal = 0;
    //On sépare l'ensemble considéré entre les valeurs supérieur et inférieur à la moyenne
    for (int i = 0 ; i<card;i++){ 
...
{ //params contient l'ensemble des indices considérés pour créer un arbre donné, indices l'ensemble des indices que l'on considére
    float Gain_max = 0.0;                                                   
    int indgainmax = 0;
    ...
    assert(Paramsenfant);
    assert(ind_gain);
    //On cherche le paramétre tel que le gain d'information soit maximal
...
{
    //si la profondeur max est atteinte ou si il n'y a plus assez de données ou de paramétres on arréte de split
    if (racine->hauteur == prof_max || racine->cardDonnee == 1||racine->cardDonnee==0||racine->cardParam==0) 
    {
        return racine;
    }
    else 
    {   //Sinon on a affecte a fils droit et fils gauche la valeur de leur split (split renvoie le Noeud dont les fils sont les deux ensembles obtenus)
...
float prediction(Noeud* abr,float Indices_test[],float data[MAX_ROWS][MAX_COLS])
{
    ...
    else
    {
        if(Indices_test[abr->indParams[abr->paramSplit]] < abr->seuil)
        {
            ...
            {
                //S'il n'y as pas assez de donnée on renvoie la classe majoritaire du noeud précédent
                for(int i = 0;i<abr->cardDonnee;i++)
                {
                    int predict = (int)data[abr->indexDonnee[i]][4];
                    tab[predict] += 1.;
...
{
    //Meme principe que prediction mais on interroge chaque arbre
    float tab[3] = {0.,0.,0.};
    int max = 0;
    for(int i = 0;i<foret->nbEstimateur;i++)
    {
